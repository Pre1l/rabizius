    ...
    public Page render(RenderData renderData) {
        ...
        for (int i = 0; i < buttonIcons.length; i++) {
            ...
            button.addActionListener(e -> {
                ...
                SubmitResult result = Core.instance().submit("/withdraw", submit);
                if (result.getStatus()) {
        ...
        withdrawPanel.setPreferredSize(new Dimension(400, 65));
        JLabel withdrawLabel = new JLabel("Withdraw");
        withdrawLabel.setForeground(PRIMARY_COLOR);
        withdrawLabel.setHorizontalAlignment(JLabel.CENTER);
        withdrawLabel.setFont(POPPINS_FONT_BOLD_L);
        withdrawPanel.add(withdrawLabel);
        middlePanel.add(withdrawPanel, c);
